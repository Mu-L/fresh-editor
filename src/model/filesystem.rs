...
//! layer to work with different backends (native filesystem, browser storage, etc.)
//!
...
pub trait FileReader: io::Read + io::Seek {}

/// Wrapper around std::fs::File that implements FileReader
struct StdFileReader(std::fs::File);

impl io::Read for StdFileReader {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        self.0.read(buf)
    }
}

impl io::Seek for StdFileReader {
...
/// Helper to get a temp path for atomic writes
